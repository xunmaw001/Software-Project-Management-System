        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '技术名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.jishuName" placeholder="技术名称" clearable></el-input>
                    </el-form-item>
                                                                                 
                    <el-form-item :label="contents.inputTitle == 1 ? '技术类型' : ''">
                        <el-select v-model="searchForm.jishuTypes" placeholder="请选择技术类型">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('jishuCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('jishuCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('jishuCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jishuCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/ruanjianxiangmu/upload/jishuCollectionMuBan.xls"
                        >批量导入技术收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jishuCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jishuCollectionUploadSuccess"
                                :on-error="jishuCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jishuCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入技术收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jishuCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jishuCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('jishuCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jishuCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.jishuCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jishuCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jishuCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jishuCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jishu_collection',//当前表表名,
                        sumColum : 'jishu_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jishu_collection_name',//分组字符串字段
                        // types : 'jishu_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "jishu_collection_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['jishuCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "jishuCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            jishuCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jishuCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            jishuCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "jishuCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            jishuCollectionTypes:jishuCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
