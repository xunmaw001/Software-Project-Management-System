    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(xiangmu !=null){
            ...
            //级联表 项目经理
            //级联表
            XiangmujingliEntity xiangmujingli = xiangmujingliService.selectById(xiangmu.getXiangmujingliId());
            if(xiangmujingli != null){
            BeanUtils.copyProperties( xiangmujingli , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "xiangmujingliId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setXiangmujingliId(xiangmujingli.getId());
            }
            //修改对应字典表字段
    ...
    */
    ...
    public R save(@RequestBody XiangmuEntity xiangmu, HttpServletRequest request){
        ...
        if(false)
            ...
            xiangmu.setXiangmujingliId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<XiangmuEntity> queryWrapper = new EntityWrapper<XiangmuEntity>()
            .eq("xiangmujingli_id", xiangmu.getXiangmujingliId())
        ...
        if(xiangmuEntity==null){
            xiangmu.setXiangmuShengyuJine(xiangmu.getXiangmuJine());
...
//                            xiangmuEntity.setXiangmujingliId(Integer.valueOf(data.get(0)));   //项目经理 要改的
...
//                            xiangmuEntity.setXiangmuJine(data.get(0));                    //项目金额 要改的
//                            xiangmuEntity.setXiangmuShengyuJine(data.get(0));                    //剩余金额 要改的
//                            xiangmuEntity.setXiangmuFile(data.get(0));                    //项目附件 要改的
        ...
        XiangmuEntity xiangmu = xiangmuService.selectById(id);
            if(xiangmu !=null){
                ...
                //级联表
                    XiangmujingliEntity xiangmujingli = xiangmujingliService.selectById(xiangmu.getXiangmujingliId());
                if(xiangmujingli != null){
                    BeanUtils.copyProperties( xiangmujingli , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setXiangmujingliId(xiangmujingli.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
